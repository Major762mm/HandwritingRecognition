...
def generate_embeddings(model, images):
    # Verifique a forma das imagens
    print("Forma das imagens antes da expansão:", images.shape)

    # Expandir dimensão para compatibilidade com o modelo
    if images.ndim == 3:  # Se as imagens estão em grayscale
        images_expanded = np.expand_dims(images, axis=-1)  # Adiciona a dimensão de canal
        images_expanded = np.repeat(images_expanded, 3, axis=-1)  # Transforma em RGB
    elif images.ndim == 4:  # Se já estão em RGB
